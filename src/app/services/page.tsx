        ...
        <ScrollAnimation animation="fadeUp">
          ...
          >
            {services.map((service: any, index: number) => (
              <div 
                key={index}
                className="group bg-gradient-to-br from-gray-900/40 to-gray-800/20 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-gray-800/50 hover:border-gray-700/50 transition-all duration-300 hover:shadow-lg"
              >
                ...
                <div className="space-y-3">
                  <p className="text-xs sm:text-sm font-semibold text-gray-300 uppercase tracking-wide">Key Features:</p>
                  <ul className="space-y-2">
                    {service.features.map((feature: string, featureIndex: number) => (
                      <li key={featureIndex} className="flex items-center text-sm text-gray-400">
                        <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-gray-600 rounded-full mr-3 flex-shrink-0"></span>
                        {feature}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mt-12 sm:mt-16 md:mt-20 text-center">
            ...
            <a 
              href="/contact" 
              className="inline-block px-8 sm:px-10 py-3 sm:py-4 bg-gray-800/60 hover:bg-gray-700/60 text-white font-semibold rounded-lg border border-gray-700/50 hover:border-gray-600/50 transition-all duration-300 shadow-lg hover:shadow-xl"
            >
              Get In Touch
            </a>
          </div>
